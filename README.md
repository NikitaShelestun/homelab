# Homelab
Hands-on home lab for Linux administration, networking and Cyber Security tools and security strategies.

## Hardware
- VirtualBox on the host PC
- Kali Linux Server (Virtual Machine)

|    Component    |   Detail    |
| --------------- | ----------- |
|     Host OS     | Windows 10  |
|    Hypervisor   | VirtualBox  |
|     Guest OS    | Kali Linux  |
| Version control | Git, GitHub |

## Projects
**Dual Server HTTPS** - [View](projects/dual-server-https/)
- Nginx and Apache2 running simultaneously on one VM, both with HTTPS, on separate ports. Resolved port conflicts, generated self-signed certificate, documented the setup.
